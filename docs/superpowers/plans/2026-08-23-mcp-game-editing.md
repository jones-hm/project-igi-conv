...
- [ ] **Step 1: Write the failing registry and dispatcher tests**
...
- [ ] **Step 3: Implement the registry and dispatcher seam**
...
- [ ] **Step 4: Run the focused tests and existing help/version smoke checks**
...
- CLI `qsc list-objects <input.qsc> [-o <output.json>]` reports task id/class/name/direct arguments and known HumanSoldier fields.
- CLI `qsc edit-object <input.qsc> -o <output.qsc> [--task-id N|--class C|--name NAME] [--position X,Y,Z] [--gamma RADIANS] [--model-id ID] [--team N] [--bone-hierarchy N] [--stand-animation N] [--set-index N=LITERAL]...` requires exactly one selected task and an explicit output path.

- [ ] **Step 1: Write failing tests for selection, movement, rotation, model ID, generic parameters, and safety**
...
- [ ] **Step 2: Run the focused editor tests and observe the expected failure**
...
- [ ] **Step 3: Implement balanced Task_New scanning and span replacement**
...
- [ ] **Step 4: Implement named CLI fields and JSON list output**

Map named HumanSoldier fields to direct indexes only after confirming class layout; use `--set-index` for other classes. Never infer a selector from a GUI setting or current working-directory state.

- [ ] **Step 5: Run focused tests, CLI smoke tests, and compile validation**
...
- [ ] **Step 4: Implement game command execution and object-edit tool adapters**
...
- [ ] **Step 5: Run protocol-level smoke tests against a real process**
...
- [ ] **Step 2: Document the complete capability registry**
...
- [ ] **Step 3: Update changelog and supported-format matrix**
